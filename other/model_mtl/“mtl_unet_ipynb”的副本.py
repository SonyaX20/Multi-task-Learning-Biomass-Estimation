...
class MultiTaskDataset(Dataset):
    def __init__(self, images: np.ndarray, labels: np.ndarray):
        ...
        if images.shape[1] < 4:
            raise ValueError(
                f"Expected at least 4 channels (1 CHM + 3 bands), got C={images.shape[1]}"
            )

        # S1 inputs: bands 1,2,3 → clamp to [S1_MIN_THRESHOLD, S1_MAX_THRESHOLD]
        inputs = images[:, 1:4, ...].astype(np.float32)
        inputs = np.clip(inputs, S1_MIN_THRESHOLD, S1_MAX_THRESHOLD)
        self.inputs = torch.from_numpy(inputs)

        # CHM: band 0 → mask out-of-range to NaN
...
model = MLPRegressor(in_channels=train_ds.inputs.shape[1]).to(device)
...
for epoch in range(1, num_epochs + 1):
    ...
    print(f"Epoch {epoch}: train {train_loss:.4f}, val loss: {loss:.4f}, R2: {r2:.4f}".format(**val_metrics))
...
model = MultiTaskMLP(
    in_channels=in_channels,
    num_classes=num_classes,
    height=train_ds.inputs.shape[2],
    width=train_ds.inputs.shape[3],
