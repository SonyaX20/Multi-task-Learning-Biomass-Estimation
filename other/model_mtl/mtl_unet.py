...
Original file is located at
    https://colab.research.google.com/drive/16HXky5p4eSeozlp4Q-sQg7m2sxJEvcei
"""
...
# Specialized training / evaluation helpers
...
def train_one_epoch_multi(model, loader, cls_criterion, reg_criterion, optimizer, device, epoch=None, lr_history=None, scheduler=None):
    return train_one_epoch(model, loader, cls_criterion, reg_criterion, optimizer, device, epoch=epoch, lr_history=lr_history, scheduler=scheduler)


def evaluate_multi(model, loader, cls_criterion, reg_criterion, device, desc="Eval Multi"):
    return evaluate(model, loader, cls_criterion, reg_criterion, device, desc=desc)
...
patch_height = train_ds.inputs.shape[2]
patch_width = train_ds.inputs.shape[3]
if configs.get("model_type", "MultiTask") == "MLP":
    model = MultiTaskMLP(
        in_channels=in_channels,
        num_classes=num_classes,
        height=patch_height,
