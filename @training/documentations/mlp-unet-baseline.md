...
To establish baseline performance for both regression (CHM height prediction) and classification (tree species) tasks:

1. **Primary Baseline Validation**: Evaluate whether MLP-based models can achieve performance comparable to the TreeSatAI benchmark results, serving as a sanity check for our data pipeline and training setup.

2. **Architecture Comparison**: Determine whether U-Net's spatial feature extraction provides advantages over MLP for SAR-based forest parameter retrieval.

3. **Learning Rate & Scheduler Strategy**: Identify optimal learning rate warmup (1e-5 → 1e-4) and cosine annealing schedule parameters (T_max, patience) for stable convergence.

4. **Batch Size Selection**: Find appropriate batch sizes that balance GPU memory utilization, gradient stability, and training speed.

5. **Network Capacity**: Determine the minimum channel width (e.g., 128 vs 256) required to adequately model the SAR-CHM relationship without overfitting.
