# -*- coding: utf-8 -*-
"""treesatai-chm-multiscale

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1fsVHRz3JEDFH5XvFx30Zyzdf5AF4zpbM
"""
...
BASE_DIR = "/content/drive/MyDrive/data/treesatai-chm-npy/"
...
class SEBlock(nn.Module):
    ...
    This module performs global average pooling to obtain channel-wise
    statistics, passes them through a bottleneck MLP, and reweights the
    original feature maps by learned channel-wise gates.
    """
...
class SEDoubleConv(nn.Module):
    """DoubleConv followed by a channel-wise SE reweighting."""
...
class UNetSEClassifier(nn.Module):
    """U-Net style classifier with Squeeze-and-Excitation (SE) blocks.

    This model mirrors the UNetClassifier architecture but augments the
    encoder and decoder with SE channel attention to better exploit
    inter-channel dependencies in the Sentinel-1 feature maps.
    """
    ...
    def __init__(
        ...
        # Encoder with SE-enhanced blocks
        self.enc1 = SEDoubleConv(in_channels, base_channels, reduction=se_reduction)
        ...
        self.enc2 = SEDoubleConv(base_channels * 2, base_channels * 2, reduction=se_reduction)
        ...
        self.bottleneck = SEDoubleConv(base_channels * 4, base_channels * 4, reduction=se_reduction)

        # Decoder (no SE here, SE is used only in the encoder/bottleneck)
        self.up2 = Up(
            in_channels=base_channels * 4,
            out_channels=base_channels * 2,
            kernel_size=3,
...
def inference_mtl(model, best_state, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader, cls_criterion=cls_criterion, reg_criterion=reg_criterion, device=device):
    if best_state is not None:
        model.load_state_dict(best_state)
...
plot_curves(history, task="cls")

inference(model, best_state, task="cls")
...
model = UNetSEClassifier(in_channels=3, num_classes=num_classes, base_channels=configs["base_channels"], dropout=configs["dropout"]).to(device)
criterion = nn.BCEWithLogitsLoss(weight=class_weights.to(device))
optimizer = torch.optim.AdamW(model.parameters(), lr=configs["max_lr"], weight_decay=configs["weight_decay"])
scheduler = None
num_epochs = configs["epochs"]
...
for epoch in range(1, num_epochs + 1):
    ...
    if epochs_no_improve >= patience:
        print("\tEarly stopping triggered after", epoch, "epochs")
        break
best_epoch = history["epoch"][-1] - epochs_no_improve
